...
page_init(void)
{
	...
	// Set the Base memory to be free
	...
	// Create an int to find the middle point of base memory and npages to
	// clear more
	int middle = 0;
	middle = (int)ROUNDUP(((char *)pages) + (sizeof(struct PageInfo) * npages) - 0xf0000000, PGSIZE) / PGSIZE;

	// Now free the remaining pages
...
page_alloc(int alloc_flags)
{
	...
	if (page_free_list) {
		struct PageInfo *tmp = page_free_list; 	// Store what page_free_list point to the tmp
...
{
	// Fill this function in
	int pgdir_index = 0;												// Create an int to hold the page directory
	...
	if (!(*pde & PTE_P)) {														// If the entry does not exist in both, create one
		if (create == 1) {														// If create flag is 1, create a new page table
			...
			*pde = page2pa(pp_page_table) | PTE_P | PTE_U | PTE_W;	// Update page to hold new page table
			page_table = (pte_t *) KADDR(page2pa((struct PageInfo *) pp_page_table));
		}

		else {																		// Else if flag is 0, return NULL
			return NULL;
		}
	}

	else {
		// If valid, get the page table
...
page_insert(pde_t *pgdir, struct PageInfo *pp, void *va, int perm)
{
	...
	else {													// If page can allocate
		physaddr_t pa = page2pa(pp);					// Get the physical address of pp 
		if (*p_pte & PTE_P)	{							// If page valid, and collide, remove it

			if (pa == PTE_ADDR(*p_pte)) {				// Check if same pa inserted at same va
				*p_pte = pa | perm | PTE_P;
				return 0;
			}

			tlb_invalidate(pgdir, va);					// Validate if page was formerly present at va
...
{
	// Fill this function in
	pte_t *p_pte;														// Create a page table entry var
	...
	*p_pte = 0;
	tlb_invalidate(pgdir, va);	// TLB must invalidate if remove an entry from page*/
}
